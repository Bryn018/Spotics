...
import { Play, Sparkles, Calendar, TrendingUp, Music, Headphones, Zap, Star, Radio, Disc3 } from 'lucide-react';
import { motion } from 'framer-motion';
import { useState } from 'react';
import { StoryDialog } from './StoryDialog';
import type { WrapReport, DailyWrapPayload, WeeklyWrapPayload, YearlyWrapPayload } from '../types';

interface WrappedSelectorProps {
  dailyWrap: WrapReport<DailyWrapPayload> | null | undefined;
  weeklyWrap: WrapReport<WeeklyWrapPayload> | null | undefined;
  yearlyWrap: WrapReport<YearlyWrapPayload> | null | undefined;
}

export function WrappedSelector({ dailyWrap, weeklyWrap, yearlyWrap }: WrappedSelectorProps) {
  ...
  const getWrapData = () => {
    if (activeWrap === 'daily') return dailyWrap?.payload ?? null;
    if (activeWrap === 'weekly') return weeklyWrap?.payload ?? null;
    return yearlyWrap?.payload ?? null;
  };
  ...
  const getStats = () => {
    if (activeWrap === 'daily' && dailyWrap?.payload) {
      ...
      return {
        ...
        description: (
          <>
            You listened to <strong>{intro?.totalTracks ?? 0} songs</strong> today.
          </>
        ),
        stats: [
          { label: 'Songs', value: String(intro?.totalTracks ?? 0), icon: Music },
          { label: 'Minutes', value: String(intro?.totalMinutes ?? 0), icon: Headphones },
          { label: 'Genre', value: intro?.topGenre ?? '—', icon: Zap }
        ]
      };
    }
    if (activeWrap === 'weekly' && weeklyWrap?.payload) {
      ...
      return {
        ...
        description: (
          <>
            <strong>{intro?.totalTracks ?? 0} tracks</strong> across <strong>{intro?.uniqueArtists ?? 0} artists</strong> this week.
          </>
        ),
        stats: [
          { label: 'Tracks', value: String(intro?.totalTracks ?? 0), icon: Music },
          { label: 'Artists', value: String(intro?.uniqueArtists ?? 0), icon: Headphones },
          { label: 'Hours', value: String(intro?.totalHours ?? 0), icon: Zap }
        ]
      };
    }
    if (activeWrap === 'yearly' && yearlyWrap?.payload) {
      ...
      return {
        ...
        description: (
          <>
            An epic year of music. <strong>{intro?.totalTracks ?? 0} songs</strong> from <strong>{intro?.totalArtists ?? 0} artists</strong> shaped your soundtrack.
          </>
        ),
        stats: [
          { label: 'Songs', value: String(intro?.totalTracks ?? 0), icon: Music },
          { label: 'Artists', value: String(intro?.totalArtists ?? 0), icon: Headphones },
          { label: 'Hours', value: String(intro?.totalHours ?? 0), icon: Zap }
        ]
      };
    }
    ...
    return {
      title: 'Look Back At It',
      description: <>Connect with Spotify to see your listening stats.</>,
      stats: [
        { label: 'Songs', value: '—', icon: Music },
        { label: 'Artists', value: '—', icon: Headphones },
        { label: 'Hours', value: '—', icon: Zap }
      ]
    };
  };
      ...
      <div className="relative overflow-hidden rounded-3xl shadow-2xl">
        <div className="relative bg-black min-h-[500px] lg:min-h-[600px] overflow-hidden">
          ...
          <motion.div 
            ...
            animate={{ scale: [1.2, 1, 1.2], x: [0, -30, 0], opacity: [0.15, 0.25, 0.15] }}
                    ...
                    <div className="relative">
                      ...
                      <div className="relative w-full aspect-square max-w-md mx-auto">
                        ...
                        <motion.div 
                          ...
                          transition={{ rotate: { duration: 15, repeat: Infinity, ease: "linear" }, scale: { duration: 3, repeat: Infinity, ease: "easeInOut", delay: 1 } }}
                          style={{ transform: 'translateX(-20px) translateY(-20px)' }}
                        >
