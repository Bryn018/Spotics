/**
 * Last.fm API interaction - reliable MD5 & auth.
 * No external dependencies.
 */

// ---- Minimal but correct MD5 (based on blueimp's JavaScript-MD5) ----
function md5(s) {
  ...
  function ii(a, b, c, d, x, s, t) { return cmn(c ^ (b | (~d)), a, b, c, d, x, s, t); }
  ...
  function hex(n) {
    ...
    let h = '';
    for (let i = 0; i < 4; i++) {
      h += hexDigits.charAt((n >> (i * 8 + 4)) & 0x0F) +
           hexDigits.charAt((n >> (i * 8)) & 0x0F);
    }
    return h;
  }
...
async function handleAuthCallback() {
  ...
  // Build the signature string exactly as Last.fm requires:
  // api_key + apiKey + method + token + apiSecret
  const sigString = `api_key${CONFIG.apiKey}methodauth.getSessiontoken${token}${CONFIG.apiSecret}`;
  console.log('Signature string:', sigString);  // check this in console
